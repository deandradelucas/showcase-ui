...
export const PricingWidget: FC<PricingWidgetProps> = ({
  ...
  return (
    <div className="theme-injected border-border bg-card w-[380px] rounded-lg border-[1.6px] p-4 shadow-xl transition-colors">
      <div className="border-border bg-muted relative mb-4 flex rounded-lg border px-2 py-2">
        ...
        {(['monthly', 'yearly'] as const).map((t) => (
          <button
            ...
            className="text-foreground relative z-10 w-1/2 cursor-pointer py-1 text-base font-bold transition-colors focus:outline-none"
          >
        ...
        {plansData[billing].map((item) => {
          ...
          return (
            <motion.button
              ...
              className="border-border bg-card relative flex h-[82px] w-full cursor-pointer items-center justify-between rounded-lg border-[1.6px] px-4 text-left transition-all focus:outline-none"
            >
              {isActive && (
                <motion.div
      ...
      <button className="bg-primary text-primary-foreground mt-5 w-full cursor-pointer rounded-lg py-3 font-semibold transition-all hover:opacity-90">
        Get Started
      </button>
    </div>
  );
