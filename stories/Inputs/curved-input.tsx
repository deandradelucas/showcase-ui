...
const edgeSeg = (g: Geometry, uTo: number, v: number, ltr: boolean) => {
  ...
  const rho = round2((g.R as number) - (g.dir as number) * v)
  const sweep = ltr === (g.dir as number) > 0 ? 1 : 0
...
const bentLinePath = (g: Geometry, u0: number, u1: number, v: number) =>
...
function CurvedInput({
  ...
  const handleSelect = (
    e: React.SyntheticEvent<HTMLInputElement> | React.KeyboardEvent<HTMLInputElement>
  ) => {
  ...
  const handleSurfaceClick = (e: React.MouseEvent<SVGSVGElement>) => {
    ...
    if (svg && geom && layout && textEl && display.length) {
      try {
        ...
        const target =
          scrollRef.current +
          (geom.uFromPoint(pt.x, pt.y) - layout.textStartU) / geom.uPerLen
        let best = 0
  ...
  if (geom && layout) {
    ...
    const layoutPath = bentLinePath(
      geom,
      layout.textStartU - scrollU,
      geom.W,
      vBase
    )
    const clipPath = bentRectPath(
      geom,
      layout.textStartU - 6,
      layout.textEndU + 8,
      -T / 2,
